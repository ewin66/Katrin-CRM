...
namespace ICSharpCode.Core
{
	...
	public sealed class AddInTreeNode
	{
        ...
        /// <summary>
        /// Builds the child items in this path for dictionary format. Ensures that all items have the type T.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="caller"></param>
        /// <param name="additionalConditions"></param>
        /// <returns>key = codon.id</returns>
        public Dictionary<string,T> BuildDictionaryChildItems<T>(object caller, IEnumerable<ICondition> additionalConditions = null)
        {           
            var codons = this.Codons;
            Dictionary<string, T> dictionaryItemss = new Dictionary<string, T>(codons.Count);
            List<T> lstItems = this.BuildChildItems<T>(caller);
            int index = 0;
            foreach (T item in lstItems)
            {
                if (!dictionaryItemss.ContainsKey(codons[index].Id))
                {
                    dictionaryItemss.Add(codons[index].Id, item);
                }
                else
                {
                    throw new Exception(string.Format("codon id has exist:id={0}", codons[index].Id));
                }
                index ++;
            }
            return dictionaryItemss;
        }
