...
{
    public class EditEffortCommand : AbstractCommand
    {
        public override void Run()
        {
            if (!(this.Owner is ListController)) return;
            ListController listController = (ListController)this.Owner;
            var parameters = new ActionParameters("EditEffort", Guid.Empty, ViewShowType.Show){
                    {"WorkingMode",EntityDetailWorkingMode.Edit}
                };
            App.Instance.Invoke("EditEffort", "Detail", parameters);
        }
    }
}
