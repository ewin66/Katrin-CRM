...
{
    public class NoteDoozer : IDoozer
    {
        ...
        public object BuildItem(BuildItemArgs args)
        {
            ...
            string addinFileName = AppDomain.CurrentDomain.BaseDirectory + "AddIns\\" + "Note.AddIn";
            AddIn addin = AddIn.Load(addinFileName);

            ICSharpCode.Core.Properties properies = new ICSharpCode.Core.Properties();
            //properies.Set<string>("id", "Note");
            properies.Set<string>("label", "${res:Note}");
            properies.Set<string>("class", "Katrin.Win.NoteModule.Note.NoteListView");
