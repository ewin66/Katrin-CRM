...
namespace Katrin.AppFramework
{
	...
	public class SafeExecutor {
		...
		public static bool TryExecute(Action method) {
			try {
				method();
				return true;
			}
        ...
        public void Execute(Action method, object targetObject, string targetObjectIdentifier)
        {
			try {
				method();
			}
        ...
        public void ExecuteOnUIThread(Action method)
        {
            System.Threading.SynchronizationContext.Current.Send(o =>
            {
                Execute(method, null, null);
            }, null);
        }
	...
	public class Tracing : IDisposable {
		...
		[Obsolete("Use 'Initialize(string outputDirectory, string defaultLevel)' instead."), Browsable(false), EditorBrowsable(EditorBrowsableState.Never)]
		public static string NUnitEnvironmentLevel {
			get { return defaultLevel; }
