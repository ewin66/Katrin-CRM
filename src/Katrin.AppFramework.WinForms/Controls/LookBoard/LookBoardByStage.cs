    ...
    public partial class LookBoardByStage : DevExpress.XtraEditors.XtraUserControl {
        ...
        public BindingList<object> BoardDataList
        {
            ...
            set {
                ...
                _bindingSource.DataSource = this.boardDataList;
                foreach (var board in _boardsList)
                {
                    board.Value.GridControl.DataSource = _bindingSource;
                    board.Value.GridControl.RefreshDataSource();
                }
                _bindingSource.CurrentItemChanged += (sender, e) =>
                {
                    if (OnCurrentChange != null)
                        OnCurrentChange(sender, new EventArgs<object>(_bindingSource.Current));
                };
            }
        }
        #endregion
        ...
        private void gridControl_DragDrop(object sender, DragEventArgs e) {
            ...
            if (propertyInfo != null)
            {
                ...
                int originalStatus = int.Parse(rowDropped.GetType().GetProperty("StatusCode").GetValue(rowDropped,null).ToString());
