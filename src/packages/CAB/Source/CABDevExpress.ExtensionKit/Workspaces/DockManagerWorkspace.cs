...
namespace CABDevExpress.Workspaces
{
    ...
    public class DockManagerWorkspace : Workspace<Control, DockManagerSmartPartInfo>
    {
        ...
        {
            dockPanel = dockManager.Panels[smartPartInfo.ID];
            if (dockPanel == null)
            {
                if (!string.IsNullOrEmpty(smartPartInfo.ParentPanelName))
                {
                    foreach (DockPanel dockRootPanel in dockManager.RootPanels)
                    {
                        if (dockRootPanel.Name == smartPartInfo.ParentPanelName)
                            // could we use Parents ID instead of Name?
                        {
                    ...
                    //If the panel is not found, just create one
                }
                else
                {
                    dockPanel = dockManager.AddPanel(smartPartInfo.Dock);
        ...
        protected static void SetDockPanelProperties(DockPanel dockPanel, DockManagerSmartPartInfo info)
        {
            ...
            if (info.XtraBounds != System.Drawing.Rectangle.Empty)
                dockPanel.Bounds = info.XtraBounds;
        }
        ...
        {
            var existingPanel = dockManager.Panels[smartPartInfo.ID];
